...
@interface YCHighLightButton : UIButton

@property (strong, nonatomic) UIColor *highlightedColor;

@end

@implementation YCHighLightButton
...
@interface YCAlertAction ()

@property (copy, nonatomic) void(^actionHandler)(YCAlertAction *action);
...
+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(YCAlertAction *action))handler {
    YCAlertAction *instance = [YCAlertAction new];
...
@interface YCAlertViewController ()
{
    UIView *_shadowView;
    UIView *_contentView;
...
@implementation YCAlertViewController

+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message {
    
    YCAlertViewController *instance = [YCAlertViewController new];
    instance.title = title;
...
- (void)creatAllButtons {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        YCHighLightButton *btn = [YCHighLightButton new];
        btn.tag = 10+i;
        ...
        [btn addTarget:self action:@selector(didCliYCButton:) forControlEvents:UIControlEventTouchUpInside];
...
#pragma mark - 事件响应
- (void)didCliYCButton:(UIButton *)sender {
    YCAlertAction *action = self.actions[sender.tag-10];
